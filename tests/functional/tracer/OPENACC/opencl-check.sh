...
EXTRAE_CONFIG_FILE=extrae.xml ./openacc-check

# Actual comparison
NENTRIES=`grep :64200000:1 openacc-check.prv | wc -l`
NEXITS=`grep :64200000:0 openacc-check.prv | wc -l`
...
NEVENTSINACCEL=`grep ^2:2:1:1:2: openacc-check.prv | wc -l`
# 11 are:
...
NSTATESINACCEL=`grep ^1:2:1:1:2 openacc-check.prv | wc -l`
# 6 are:
...
diff openacc-check.pcf openacc-check.pcf
